...
**Scoped down during implementation planning** (see the linked plan below for full reasoning): `DamageTaken` events carry *raw* pre-mitigation damage, not net — historical mitigated-HP snapshots aren't reconstructible from the current map-only read API. Only XP-token receipts become `RewardReceived` events — confirmed a structural fact, not just an unresolved ambiguity: on a successful attack the construct sends the XP token to the attacking character's own account, but sends the HP token (the other reward currency) to the character's *creator's* EOA directly, bypassing the character's account entirely. A per-construct "damage dealt" figure is therefore a player-profile-level feed, out of scope for a method keyed to one character.

**Legitimacy filtering added during implementation planning** (Oliver's review, 2026-07-25): a transaction is only narrated if it matches one of three *verified* identity patterns — sender is the character's own creator (owner action), sender's **live codehash** matches the gamemaster registry's current trusted construct hash (construct attack — not just "the message looks like RECEIVE_ATTACK"), or sender is the character itself and recipient is its creator (the contract's own outgoing narrative text, e.g. "Congrats, character leveled up" — a new `CharacterMessage` event type, added on the same review pass). Anything else is silently dropped rather than guessed at.
