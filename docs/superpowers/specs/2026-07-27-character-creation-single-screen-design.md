...
- `hooks/useCharacterCreationProgress.ts` (refactored): `useQuery(['characterProgress', contractId], () => pollOnce(ledger, tx1Id, tx2Id), { refetchInterval: 15_000, enabled: !!tx1Id && !!tx2Id && state !== 'live', refetchOnWindowFocus: false })`. Whichever component mounts first starts the interval; if both Header (indirectly, see below) and the character page are mounted, they share the single interval and cache entry — the existing dedup guarantee react-query already provides elsewhere in this codebase.
- Header does **not** need to run `useCharacterCreationProgress` itself for every pending character just to show a dot — it only needs `usePendingCharacters()`'s cached `state` field (written back to localStorage by whichever component *is* actively polling — normally the character page, if the user has it open; if not, no page is polling that character's chain state right now, and the badge shows the last-known state until the user opens that character's page again).

This is a conscious trade-off surfaced during design, not an oversight:

> **True "polls even when no page is open" tracking would require *something* to always mount the progress query — e.g. a tiny invisible `<CreationTracker contractId />` rendered inside `Header.tsx` for every pending, non-terminal entry from `usePendingCharacters()`.** This is cheap (no visible UI, just a hook call per pending character) and is the recommended default: it makes the badge's state genuinely live site-wide, matching the "persists across navigation" requirement precisely. Implement it this way — `Header.tsx` maps over non-terminal pending entries and renders one `<CreationTracker key={contractId} contractId={tx1Id} tx2Id={tx2Id} />` (a component with no return value beyond `null`, existing purely to keep the `useCharacterCreationProgress` query alive and its `updatePendingCharacter` side effect running) per entry.
...
- Wallet rejects/cancels signing before any `contractId` exists → stay on `/character/create`, inline error (unchanged from today's cancellation handling in `useCharacterCreation`), no pending entry written.
