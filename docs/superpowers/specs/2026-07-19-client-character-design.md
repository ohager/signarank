...
- The character codehash changes (Part 1). At deploy, update **`G_CHARACTER_HASH`** on the gamemaster registry and the **trusted character hash** on the char-account registry to the new value. Pre-launch, no live characters depend on the old layout.
  - Current character codehash: **`14359912628548892607`** (code size 9489/10240 bytes = 92.7%). Set this as `G_CHARACTER_HASH` and the char-account-registry trusted character hash; it also feeds `CharacterDataFieldIndex` derivation in Part 2 Task 10. Reflects: VITALS + effective sta/dex/will in COMBAT + STATUS_EFFECT_ID, the DEDUCT_HITPOINTS/COMBAT → unified RECEIVE_ATTACK(13) consolidation, and the combat-profile republish cache (see D16). (Superseded values: `8336535780851164957` pre-RECEIVE_ATTACK; `1535226251089564205` pre-republish-cache.)
...
- **D16** Combat-profile republish is cached, not unconditional. `publishCombatProfile()` (~34 map ops ≈ the single largest per-activation cost) is gated behind `combatDirty` (set at the 7 sites that mutate attributes/equipment/status/primaryAttackEffectId) + a `combatValidUntil` watermark (earliest active profile-status expiry) so a time-based lapse still forces a recompute. Skips the republish on the hot paths (attack, construct counter-attack, deposits, transfers, poke): clean activation 1.387 → 0.445 SIGNA, attack ~1.4 → 0.687, counter ~1.53 → 0.708. Rare write paths (useItem/reroll/allocate/equip) pay a small watermark-scan overhead. Vitals + progression stay unconditional (cheap). DamageTaken/HP/inv-slot statuses are excluded from the watermark (they don't feed the profile).
